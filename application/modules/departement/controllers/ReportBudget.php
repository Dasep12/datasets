...
class ReportBudget extends CI_Controller
{
    ...
    public function planTahunanPDF()
    {
        $mpdf            = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'A3-L']);
        $data = [];
        $view = $this->load->view("pdfTahunan", $data, TRUE);
        $mpdf->WriteHTML($view);
        $mpdf->Output("Report Regular.pdf", 'I');
    }

    public function planBulananPDF()
    {
        ...
        $mpdf            = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'A3-L']);
        $depta           = $this->db->query("SELECT nama_departement as name FROM master_departement WHERE id = '" . $dept . "'  ")->row();
        $head = $this->db->query("SELECT mb.id_budget as id , mb.kode_budget as code , mb.description , ma.acc_name ,
            (select sum(mpb.nilai_budget) ) as total
            from master_budget mb 
            left join  master_planning_budget mpb on mpb.master_budget_id_budget = mb.id_budget 
            left join master_acc ma on ma.id  = mb.account_bame 
            where mb.departement_id  = '" . $dept . "' and mb.approve_spv  = 1  and mb.master_jenis_budget_id  = $jenis
        ...
        $data = [
            ...
            'manager'           => $this->db->query("SELECT  nama_lengkap , mt.file FROM master_akun ma 
            ...
            'bc'            => $this->model->lisTertanda("BC")->row(),
            'gm' =>  $this->model->lisTertanda("GM")->row(),
            'fin' =>  $this->model->lisTertanda("FIN")->row(),
            'date'  => 'Bekasi ' . date('d , ') . ucwords(strtolower(convertbulan(date('m')))) . date(' Y')

        ];
        $view = $this->load->view("pdfPlantBulanan", $data, TRUE);
        $mpdf->WriteHTML($view);
        $mpdf->Output("Report Regular.pdf", 'I');
    }
}
